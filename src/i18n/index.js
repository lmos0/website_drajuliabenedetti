...
import { setText, setHtml, setAttr } from "../utils/dom.js";
...
export function updateLanguageLinks() {
  ...
  document.querySelectorAll(".language-option[lang='pt-BR']").forEach((link) => {
    ...
    link.classList.toggle("active", getLang() === "pt");
    link.toggleAttribute("aria-current", getLang() === "pt");
  });

  document.querySelectorAll(".language-option[lang='en']").forEach((link) => {
    link.setAttribute("href", englishHref);
    link.classList.toggle("active", getLang() === "en");
    link.toggleAttribute("aria-current", getLang() === "en");
  });
}
